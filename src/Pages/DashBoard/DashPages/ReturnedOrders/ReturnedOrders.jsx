...
const ReturnedOrders = () => {
  ...
  const returned = allOrders.filter((p) => p?.status === "returned");
    ...
    <div className="pt-4 sm:px-6 md:px-10 space-y-6 md:space-y-10">
      ...
      {returned.length === 0 ? (
        <div className="text-center min-h-[50vh] flex flex-col justify-center items-center space-y-3">
          <p className="font-bold text-xl text-gray-500">No orders have been returned yet.</p>
        </div>
      ) : (
            ...
            <table className="min-w-full shadow-md divide-y divide-gray-200 rounded-lg">
              <thead className="bg-gray-800 text-white sticky top-0 z-50">
                <tr>
                  ...
                  <th className="py-3 px-6 text-center text-sm font-semibold">Quantity</th>
                  <th className="py-3 px-6 text-center text-sm font-semibold">Advance</th>
                  <th className="py-3 px-6 text-center text-sm font-semibold">Remaining</th>
                  <th className="py-3 px-6 text-center text-sm font-semibold">Details</th>
                  <th className="py-3 px-6 text-center text-sm font-semibold">Status</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {returned
                  ...
                  .map((item, index) => (
                    <tr key={item._id} className="hover:bg-yellow-50 transition-colors duration-200">
                      ...
                      <td className="py-4 px-6 text-center text-gray-700">৳{item.advanceAmount}</td>
                      <td className="py-4 px-6 text-center text-gray-700">৳{item.remainingAmount}</td>
