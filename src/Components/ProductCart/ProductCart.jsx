...
const ProductCart = ({ isCartOpen, setIsCartOpen }) => {
    ...
    if (isLoading) return <div className="p-6 text-black dark:text-white">Loading cart...</div>;
            ...
            <div className={`fixed top-0 right-0 h-full w-full sm:w-96 bg-white dark:bg-[#1A1A1A] shadow-lg transform ${isCartOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 ease-in-out z-50`}>
                <div className="p-6 h-full flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto">
                        <div className="border-b border-gray-200 dark:border-gray-700 py-4 space-y-8">
                            {data.map((item) => {
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="flex items-center mt-2">
                                                <button
                                                    ...
                                                    className="w-8 h-8 flex items-center justify-center border text-white border-gray-300 dark:border-gray-600 rounded-l-md hover:bg-gray-200 dark:hover:bg-gray-700 transition"
                                                >
                                                    -
                                                </button>
                                                <span className="w-10 h-8 flex items-center justify-center text-black dark:text-white border-t border-b border-gray-300 dark:border-gray-600">
                                                    {item.quantity || 1}
                                                </span>
                                                <button
                                                    onClick={() => handleQuantityChange(itemKey, (item.quantity || 1) + 1)}
                                                    className="w-8 h-8 flex items-center justify-center border text-white border-gray-300 dark:border-gray-600 rounded-r-md hover:bg-gray-200 dark:hover:bg-gray-700 transition"
                                                >
                                                    +
                                                </button>
                                            </div>
